...
* **Tailwind CSS (CDN):** Tasarım sürecini hızlandırmak, "Mobile-First" (önce mobil) yaklaşımını korumak ve kurumsal bir "SaaS" arayüzü görünümü elde etmek için kullanıldı.

### 🧠 2. Intelligence & Data (Yapay Zeka ve Veri)
* **AI Engine (Yapay Zeka Motoru):** Karmaşık üretim planlama ve kapasite optimizasyon algoritmalarını doğal dil işleme yeteneğiyle çözmek için seçildi.
* **JSON Data Management:** Üretici veritabanı (hız, müsaitlik, uzmanlık) JSON formatında yapılandırıldı. Bu, verinin hem okunabilir hem de AI tarafından kolayca manipüle edilebilir olmasını sağlar.

### 🛠️ 3. Development & Workflow (Geliştirme Araçları)
* **IDE:** **Cursor AI**. "Agentic Workflow" (AI destekli geliştirme) ile kod hatalarını minimize etmek ve PRD dökümanlarını koda hızlıca dönüştürmek için tercih edildi.
